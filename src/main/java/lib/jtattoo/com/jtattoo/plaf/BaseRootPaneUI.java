...
public class BaseRootPaneUI extends BasicRootPaneUI {
    ...
    private static class BaseRootLayout implements LayoutManager2 {
         ...
         *
         * @param parent Container for which this layout manager is being used
         ...
         *
         * @param target Container for which this layout manager is being used
...
//------------------------------------------------------------------------------    
    ...
    private class MouseInputHandler implements MouseInputListener {
            ...
            if (ev.getSource() instanceof Window) {
                ...
                if (getTitlePane() != null && getTitlePane().contains(convertedDragWindowOffset)) {
                    if ((f != null && ((frameState & BaseRootPaneUI.MAXIMIZED_BOTH) == 0) || (d != null))
                        ...
                        if (window instanceof JFrame) {
                            ...
                            for (PropertyChangeListener aPcl : pcl) {
                                aPcl.propertyChange(new PropertyChangeEvent(window, "windowMoving", Boolean.FALSE, Boolean.FALSE));
                            }
                        }
                    ...
                    if (window instanceof JFrame) {
                        ...
                        PropertyChangeListener[] pcl = frame.getPropertyChangeListeners();
                        for (PropertyChangeListener aPcl : pcl) {
                            aPcl.propertyChange(new PropertyChangeEvent(window, "windowResizing", Boolean.FALSE, Boolean.FALSE));
                        }
                    }
                    if (window instanceof JDialog) {
                        JDialog dialog = (JDialog) window;
        ...
        public void mouseReleased(MouseEvent ev) {
            if (ev.getSource() != null && ev.getSource() instanceof Window) {
                Window w = (Window) ev.getSource();
                ...
                if (window instanceof JFrame) {
                    ...
                    for (PropertyChangeListener aPcl : pcl) {
                        if (isMovingWindow) {
                            aPcl.propertyChange(new PropertyChangeEvent(window, "windowMoved", Boolean.FALSE, Boolean.FALSE));
                        } else {
                            aPcl.propertyChange(new PropertyChangeEvent(window, "windowResized", Boolean.FALSE, Boolean.FALSE));
                        }
                    }
                }
                if (window instanceof JDialog) {
