 ...
 *
 * Used by Nick a.k.a. "Gocnak" to scale Images for Botnak. All the code is
 * pertained to Buzz Media's source.
 *
...
/**
 * Class used to implement performant, high-quality and intelligent com.gocnak.image
 * scaling and manipulation algorithms in native Java 2D.
 * <p>
 * This class utilizes the Java2D "best practices" for com.gocnak.image manipulation,
 ...
 * <p>
 * <h3>Image Quality</h3>
 * This class implements a few different methods for scaling an com.gocnak.image, providing
 ...
 * .net/pub/a/today/2007/04/03/perils-of-com.gocnak.image-getscaledinstance.html">Perils of
 * Image.getScaledInstance()</a> article in order to give the best-looking com.gocnak.image
 ...
 * look at the size of the com.gocnak.image before selecting an approach to scaling the
 * com.gocnak.image. If {@link Method#QUALITY} is specified, the best-looking algorithm
 ...
 * source com.gocnak.image down, an explicit effort is made to call
 ...
 * the original com.gocnak.image unless a mode of {@link Mode#FIT_EXACT} is specified; in
 * which case the orientation and proportion of the source com.gocnak.image is ignored and
 * the com.gocnak.image is stretched (if necessary) to fit the exact dimensions given.
 * <p>
 ...
 * behavior:
 * <ol>
 * <li>If the com.gocnak.image is LANDSCAPE-oriented or SQUARE, treat the
 ...
 * <li>If com.gocnak.image is PORTRAIT-oriented, treat the <code>targetHeight</code> as the
 ...
 * the com.gocnak.image's orientation is ignored and the scaled com.gocnak.image is fit to the
 * preferred dimension by using the value passed in by the user for that
 * dimension and recalculating the other (regardless of com.gocnak.image orientation). This
 ...
 * Java2D provides support for a number of different com.gocnak.image types defined as
 * <code>BufferedImage.TYPE_*</code> variables, unfortunately not all com.gocnak.image
 * types are supported equally in the Java2D rendering pipeline.
 * <p>
 * Some more obscure com.gocnak.image types either have poor or no support, leading to
 ...
 * source com.gocnak.image. In many cases, especially when applying {@link BufferedImageOp}
 * s, using poorly supported com.gocnak.image types can even lead to exceptions or total
 * corruption of the com.gocnak.image (e.g. solid black com.gocnak.image).
 * <p>
 ...
 * </ol>
 * depending on if the source com.gocnak.image utilizes transparency or not. This is a
 * recommended approach by the Java2D team for dealing with poorly (or non)
 * supported com.gocnak.image types. More can be read about this issue <a href=
 ...
 * maintaining transparency when moving to an com.gocnak.image of type
 * {@link BufferedImage#TYPE_INT_ARGB}; because of this issue when a GIF com.gocnak.image
 * is processed by imgscalr and the result saved as a GIF file (instead of PNG),
 * it is possible to lose the alpha channel of a transparent com.gocnak.image or in the
 ...
 * com.gocnak.image. This would lead to a very measurable negative impact on performance
 ...
 * made except when the com.gocnak.image is saved out, e.g. using {@link ImageIO}.
 * <p>
 ...
 * GIF implementation.
 * <p>
 * If the issue with optional {@link BufferedImageOp}s destroying GIF com.gocnak.image
 ...
 * <h3>Thread Safety</h3>
 * The {@link Scalr} class is <strong>com.gocnak.thread-safe</strong> (as all the methods
...
public class Scalr {
     ...
     * anti-aliasing filter (softens the com.gocnak.image a bit) when applied to an com.gocnak.image.
     * <p>
     ...
     * the very edge of the com.gocnak.image (otherwise EDGE_ZERO_FILL would create a
     * black-border around the com.gocnak.image). If you have not worked with a ConvolveOp
     ...
     * com.gocnak.image every time, so no color conversion is ever needed and thus no
     * hints.
     * <h3>Performance</h3>
     * Use of this (and other) {@link ConvolveOp}s are hardware accelerated when
     * possible. For more information on if your com.gocnak.image op is hardware
     ...
     * >sun.awt.com.gocnak.image.ImagingLib</a>.
     * <h3>Known Issues</h3>
     ...
     * resulting com.gocnak.image as a GIF results in a corrupted/empty file. The file must
    ...
    /**
     * A {@link RescaleOp} used to make any input com.gocnak.image 10% darker.
     * <p>
    ...
    /**
     * A {@link RescaleOp} used to make any input com.gocnak.image 10% brighter.
     * <p>
     * This operation can be applied multiple times in a row if greater than 10%
     * changes in brightness are desired.
     *
    ...
    /**
     * A {@link ColorConvertOp} used to convert any com.gocnak.image to a grayscale color
     * palette.
     * <p>
     * Applying this op multiple times to the same com.gocnak.image has no compounding
     * effects.
     *
     * @since 4.0
     */
    ...
    public enum Method {
         ...
         * method to use in order to get the best looking scaled com.gocnak.image in the
         ...
         * for 80% of people looking to scale an com.gocnak.image quickly and get a good
         * looking result.
         */
        AUTOMATIC,
        /**
         ...
         * source-com.gocnak.image size increases. Using the AUTOMATIC method will
         * automatically prefer the QUALITY method when scaling an com.gocnak.image down
         * below 800px in size.
         */
        QUALITY,
        /**
         * Used to indicate that the scaling implementation should go above and
         * beyond the work done by {@link Method#QUALITY} to make the com.gocnak.image look
    ...
    public enum Mode {
         ...
         * dimensions for the resultant com.gocnak.image by looking at the com.gocnak.image's
        ...
        AUTOMATIC,
        /**
         * Used to fit the com.gocnak.image to the exact dimensions given regardless of the
         * com.gocnak.image's proportions. If the dimensions are not proportionally
         ...
         * modes or {@link Mode#AUTOMATIC} if you want the com.gocnak.image to look
         * correct, but if dimension-fitting is the #1 priority regardless of
         * how it makes the com.gocnak.image look, that is what this mode is for.
         */
        FIT_EXACT,
        /**
         * Used to indicate that the scaling implementation should calculate
         * dimensions for the resultant com.gocnak.image that best-fit within the given
         * width, regardless of the orientation of the com.gocnak.image.
         */
        FIT_TO_WIDTH,
        /**
         * Used to indicate that the scaling implementation should calculate
         * dimensions for the resultant com.gocnak.image that best-fit within the given
         * height, regardless of the orientation of the com.gocnak.image.
         */
        FIT_TO_HEIGHT
    }
     ...
     * com.gocnak.image during a resize operation.
     *
     * @author Riyad Kalla (software@thebuzzmedia.com)
     * @since 3.2
     */
    public enum Rotation {
         ...
         * quarter-turn of the com.gocnak.image to the right; moving the picture on to its
         * right side.
         */
        CW_90,
        /**
         * 180-degree, clockwise rotation (to the right). This is equivalent to
         * 1 half-turn of the com.gocnak.image to the right; rotating the picture around
         * until it is upside down from the original position.
         */
        CW_180,
        /**
         * 270-degree, clockwise rotation (to the right). This is equivalent to
         * a quarter-turn of the com.gocnak.image to the left; moving the picture on to its
         * left side.
         */
        CW_270,
        /**
         * Flip the com.gocnak.image horizontally by reflecting it around the y axis.
         * <p>
         * This is not a standard rotation around a center point, but instead
         * creates the mirrored reflection of the com.gocnak.image horizontally.
         * <p>
         * More specifically, the vertical orientation of the com.gocnak.image stays the
        ...
        FLIP_HORZ,
        /**
         * Flip the com.gocnak.image vertically by reflecting it around the x axis.
         * <p>
         * This is not a standard rotation around a center point, but instead
         * creates the mirrored reflection of the com.gocnak.image vertically.
         * <p>
         * More specifically, the horizontal orientation of the com.gocnak.image stays the
     ...
     * <p>
     * The bigger the com.gocnak.image is being scaled to, the less noticeable degradations
     * in the com.gocnak.image becomes and the faster algorithms can be selected.
     * <p>
     ...
     * {@link ImagingOpException}s when run against a <code>src</code> com.gocnak.image
     ...
     * {@link ConvolveOp}) are. For more information on if your com.gocnak.image op is
     ...
     * com.gocnak.image unmodified. If the caller is done with the <code>src</code> com.gocnak.image
     ...
     * resources and make it easier for the GC to collect the unused com.gocnak.image.
     *
     * @param src The com.gocnak.image that will have the ops applied to it.
     * @param ops <code>1</code> or more ops to apply to the com.gocnak.image.
     ...
     *                                  to fail, even when using straight forward JDK-com.gocnak.image
     *                                  operations.
     */
    public static BufferedImage apply(BufferedImage src, BufferedImageOp... ops)
		...
		/*
         * Ensure the src com.gocnak.image is in the best supported com.gocnak.image type before we
		 ...
		 * com.gocnak.image after failing to apply; some simply return a corrupted/black
		 * com.gocnak.image that result in no exception and it is up to the user to
		 * discover this.
		 *
		 * In internal testing, EVERY failure I've ever seen was the result of
		 * the source com.gocnak.image being in a poorly-supported BufferedImage Type like
		 * BGR or ABGR (even though it was loaded with ImageIO).
		 *
		 * To avoid this nasty/stupid surprise with BufferedImageOps, we always
		 * ensure that the src com.gocnak.image starts in an optimally supported format
        ...
        for (BufferedImageOp op1 : ops) {
			 ...
			 * because we are trying to create an com.gocnak.image big enough to hold the
			 * result of this operation (which may be to scale the com.gocnak.image
			 * smaller), in that case the bounds reported by this op and the
			 * bounds reported by the source com.gocnak.image will be different.
			 */
            ...
            if (resultBounds == null)
                throw new ImagingOpException(
                        "BufferedImageOp ["
                                + op1.toString()
                                + "] getBounds2D(src) returned null bounds for the target com.gocnak.image; this should not happen and indicates a problem with application of this type of op."
                );

			/*
             * We must manually create the target com.gocnak.image; we cannot rely on the
			...
			/*
             * Flush the 'src' com.gocnak.image ONLY IF it is one of our interim temporary
			 * images being used when applying 2 or more operations back to
			 * back. We never want to flush the original com.gocnak.image passed in.
			 */
            if (hasReassignedSrc)
                src.flush();
			 ...
			 * com.gocnak.image, so we know when we can explicitly flush it and clean up
    ...
    /**
     * Used to crop the given <code>src</code> com.gocnak.image from the top-left corner
    ...
    /**
     * Used to crop the given <code>src</code> com.gocnak.image and apply any optional
    ...
    public static BufferedImage crop(BufferedImage src, int x, int y,
        ...
        // Create a target com.gocnak.image of an optimal type to render into.
        BufferedImage result = createOptimalImage(src, width, height);
        Graphics g = result.getGraphics();

		/*
         * Render the region specified by our crop bounds from the src com.gocnak.image
		 * directly into our result com.gocnak.image (which is the exact size of the crop
		 * region).
		 */
    ...
    /**
     * Used to apply padding around the edges of an com.gocnak.image using
     ...
     * top, bottom, left and right sides of the resulting com.gocnak.image causing the
     ...
     * resources and make it easier for the GC to collect the unused com.gocnak.image.
     *
     * @param src     The com.gocnak.image the padding will be added to.
     * @param padding The number of pixels of padding to add to each side in the
     *                resulting com.gocnak.image. If this value is <code>0</code> then
    ...
    /**
     * Used to apply padding around the edges of an com.gocnak.image using the given color
    ...
    public static BufferedImage pad(BufferedImage src, int padding,
		...
		/*
         * Double the padding to account for all sides of the com.gocnak.image. More
		...
		/*
		 * We need to make sure our resulting com.gocnak.image that we render into contains
		 * alpha if either our original com.gocnak.image OR the padding color we are using
		 * contain it.
		 */
        if (colorHasAlpha || imageHasAlpha) {
            if (DEBUG)
                log(1,
                        "Transparency FOUND in source com.gocnak.image or color, using ARGB com.gocnak.image type...");
            ...
            if (DEBUG)
                log(1,
                        "Transparency NOT FOUND in source com.gocnak.image or color, using RGB com.gocnak.image type...");
        ...
        // "Clear" the background of the new com.gocnak.image with our padding color first.
        g.setColor(color);
        g.fillRect(0, 0, newWidth, newHeight);

        // Draw the com.gocnak.image into the center of the new padded com.gocnak.image.
    ...
    /**
     * Resize a given com.gocnak.image (maintaining its original proportion) to a width and
     ...
     * resources and make it easier for the GC to collect the unused com.gocnak.image.
     *
     * @param src        The com.gocnak.image that will be scaled.
     * @param targetSize The target width and height (square) that you wish the com.gocnak.image
     *                   to fit within.
     * @param ops        <code>0</code> or more optional com.gocnak.image operations (e.g.
     *                   sharpen, blur, etc.) that can be applied to the final result
     *                   before returning the com.gocnak.image.
     * @return a new {@link BufferedImage} representing the scaled
     * <code>src</code> com.gocnak.image.
     * @throws IllegalArgumentException if <code>src</code> is <code>null</code>.
     ...
     *
     * @param src           The com.gocnak.image that will be scaled.
     * @param scalingMethod The method used for scaling the com.gocnak.image; preferring speed to
    ...
    /**
     * Resize a given com.gocnak.image (maintaining its original proportion) to a width and
     * height no bigger than <code>targetSize</code> (or fitting the com.gocnak.image to
     ...
     *                   target size for the com.gocnak.image, either fitting the com.gocnak.image to the
     *                   given width ({@link Mode#FIT_TO_WIDTH}) or fitting the com.gocnak.image
     *                   to the given height ({@link Mode#FIT_TO_HEIGHT}). If
     *                   {@link Mode#AUTOMATIC} is passed in, imgscalr will calculate
     *                   proportional dimensions for the scaled com.gocnak.image based on its
    ...
    /**
     * Resize a given com.gocnak.image (maintaining its original proportion) to the target
     ...
     * <code>targetHeight</code> depending on the com.gocnak.image's orientation in order
     ...
     *
     * @param src          The com.gocnak.image that will be scaled.
     * @param targetWidth  The target width that you wish the com.gocnak.image to have.
     * @param targetHeight The target height that you wish the com.gocnak.image to have.
     * @param ops          <code>0</code> or more optional com.gocnak.image operations (e.g.
     *                     sharpen, blur, etc.) that can be applied to the final result
     *                     before returning the com.gocnak.image.
     * @return a new {@link BufferedImage} representing the scaled
    ...
    /**
     * Resize a given com.gocnak.image (maintaining its original proportion) to the target
     * width and height (or fitting the com.gocnak.image to the given WIDTH or HEIGHT
    ...
    public static BufferedImage resize(BufferedImage src, Method scalingMethod,
        ...
        // <= 1 is a square or landscape-oriented com.gocnak.image, > 1 is a portrait.
		 ...
		 * of FIT_EXACT, ignore com.gocnak.image proportions and orientation and just use
		 * what the user sent in, otherwise the proportion of the picture must
		 * be honored.
		 *
		 * The way that is done is to figure out if the com.gocnak.image is in a
		 ...
		 * just specify the dimensions they would like the com.gocnak.image to roughly fit
        ...
        if (resizeMode != Mode.FIT_EXACT) {
            if ((ratio <= 1 && resizeMode == Mode.AUTOMATIC)
                ...
                if (DEBUG && originalTargetHeight != targetHeight)
                    log(1,
                            "Auto-Corrected targetHeight [from=%d to=%d] to honor com.gocnak.image proportions.",
                ...
                if (DEBUG && originalTargetWidth != targetWidth)
                    log(1,
                            "Auto-Corrected targetWidth [from=%d to=%d] to honor com.gocnak.image proportions.",
        ...
        // Now we scale the com.gocnak.image
        if (scalingMethod == Scalr.Method.SPEED) {
			 ...
			 * the com.gocnak.image will stay proportional we just check if either are
            ...
            if (targetWidth > currentWidth || targetHeight > currentHeight) {
				 ...
				 * "softening" in the resultant com.gocnak.image with BILINEAR that would
				 * be unexpectedly annoying to a user expecting a "QUALITY"
				 * scale of their original com.gocnak.image. Instead BICUBIC was chosen to
				 * honor the contract of a QUALITY scale of the original com.gocnak.image.
				 */
     ...
     * {@link BufferedImageOp}s to a given com.gocnak.image and return the result.
     * <p>
     * <strong>TIP</strong>: This operation leaves the original <code>src</code>
     * com.gocnak.image unmodified. If the caller is done with the <code>src</code> com.gocnak.image
     * after getting the result of this operation, remember to call
     ...
     * resources and make it easier for the GC to collect the unused com.gocnak.image.
     *
     * @param src      The com.gocnak.image that will have the rotation applied to it.
     * @param rotation The rotation that will be applied to the com.gocnak.image.
     * @param ops      Zero or more optional com.gocnak.image operations (e.g. sharpen, blur,
     *                 etc.) that can be applied to the final result before returning
     *                 the com.gocnak.image.
    ...
    public static BufferedImage rotate(BufferedImage src, Rotation rotation,
		...
		/*
		 * Setup the default width/height values from our com.gocnak.image.
		 *
		 ...
		 * than it is to re-use a single AffineTransform per-com.gocnak.thread via the
        ...
        switch (rotation) {
			 ...
			 * flip-flop from the original com.gocnak.image to the rotated one.
			 */
        ...
        // Create our target com.gocnak.image we will render the rotated result to.
        BufferedImage result = createOptimalImage(src, newWidth, newHeight);
        Graphics2D g2d = result.createGraphics();

		/*
		 * Render the resultant com.gocnak.image to our new rotatedImage buffer, applying
		 ...
		 * the resulting com.gocnak.image correctly.
		 */
     ...
     * <p>
     * This does not perform a copy of the com.gocnak.image data from <code>src</code> into
     * the result com.gocnak.image; see {@link #copyToOptimalImage(BufferedImage)} for
     * that.
     * <p>
     * We force all rendering results into one of these two types, avoiding the
     * case where a source com.gocnak.image is of an unsupported (or poorly supported)
     * format by Java2D causing the rendering result to end up looking terrible
     * (common with GIFs) or be totally corrupt (e.g. solid black com.gocnak.image).
     * <p>
     * Originally reported by Magnus Kvalheim from Movellas when scaling certain
     * GIF and PNG images.
     *
     * @param src The source com.gocnak.image that will be analyzed to determine the most
     *            optimal com.gocnak.image type it can be rendered into.
     * @return a new {@link BufferedImage} representing the most optimal target
     * com.gocnak.image type that <code>src</code> can be rendered into.
     * @see <a
     * href="http://www.mail-archive.com/java2d-interest@capra.eng.sun.com/msg05621.html">How
     * Java2D handles poorly supported com.gocnak.image types</a>
     * @see <a
     ...
     * @param src    The source com.gocnak.image that will be analyzed to determine the most
     *               optimal com.gocnak.image type it can be rendered into.
     * @param width  The width of the newly created resulting com.gocnak.image.
     * @param height The height of the newly created resulting com.gocnak.image.
     * @return a new {@link BufferedImage} representing the most optimal target
     * com.gocnak.image type that <code>src</code> can be rendered into.
     ...
     * otherwise) to convert any inbound com.gocnak.image from a poorly supported com.gocnak.image
     * type into the 2 most well-supported com.gocnak.image types in Java2D (
     * {@link BufferedImage#TYPE_INT_RGB} or {@link BufferedImage#TYPE_INT_ARGB}
     * ) in order to ensure all subsequent graphics operations are performed as
     * efficiently and correctly as possible.
     * <p>
     * When using Java2D to work with com.gocnak.image types that are not well supported,
     ...
     * black.
     *
     * @param src The com.gocnak.image to copy (if necessary) into an optimally typed
     *            {@link BufferedImage}.
     * @return a representation of the <code>src</code> com.gocnak.image in an optimally
    ...
    protected static BufferedImage copyToOptimalImage(BufferedImage src)
        ...
        // Render the src com.gocnak.image into our new optimal source.
        Graphics g = result.getGraphics();
     ...
     * scaling the com.gocnak.image to the targeted dimensions.
     * <p>
     ...
     * height) against the threshold and seeing where the com.gocnak.image falls. The
     * primary dimension is determined by looking at the orientation of the
     * com.gocnak.image: landscape or square images use their width and portrait-oriented
     * images use their height.
     *
     * @param targetWidth  The target width for the scaled com.gocnak.image.
     * @param targetHeight The target height for the scaled com.gocnak.image.
     * @param ratio        A height/width ratio used to determine the orientation of the
     *                     com.gocnak.image so the primary dimension (width or height) can be
     *                     selected to test if it is greater than or less than a
     *                     particular threshold.
     * @return the fastest {@link Method} suited for scaling the com.gocnak.image to the
    ...
    protected static Method determineScalingMethod(int targetWidth,
        ...
        // Get the primary dimension based on the orientation of the com.gocnak.image
    ...
    /**
     * Used to implement a straight-forward com.gocnak.image-scaling operation using Java
     * 2D.
     * <p>
     * This method uses the Oracle-encouraged method of
     * <code>Graphics2D.drawImage(...)</code> to scale the given com.gocnak.image with the
    ...
    protected static BufferedImage scaleImage(BufferedImage src,
        ...
        // Setup the rendering resources to match the source com.gocnak.image's
        BufferedImage result = createOptimalImage(src, targetWidth,
                targetHeight);
        Graphics2D resultGraphics = result.createGraphics();

        // Scale the com.gocnak.image to the new buffer using the specified rendering hint.
        ...
        // Return the scaled com.gocnak.image to the caller.
        return result;
    }
     ...
     * -of-com.gocnak.image-getscaledinstance
     * .html">http://today.java.net/pub/a/today/2007/04/03/perils
     * -of-com.gocnak.image-getscaledinstance.html</a>.
     * <p>
     ...
     * instances to avoid com.gocnak.image leaking.
     *
     * @param src                    The com.gocnak.image that will be scaled.
     * @param targetWidth            The target width for the scaled com.gocnak.image.
     * @param targetHeight           The target height for the scaled com.gocnak.image.
     ...
     *                               indicate the method that {@link Graphics2D} should use when
     *                               scaling the com.gocnak.image.
     * @return an com.gocnak.image scaled to the given dimensions using the given rendering
     * hint.
     */
    protected static BufferedImage scaleImageIncrementally(BufferedImage src,
		 ...
		 * is to step down by 1/2s every time when scaling the com.gocnak.image
		 ...
		 * com.gocnak.image while a divisor of 8 or higher made the resulting thumbnail too
		 ...
		 * do a linear reduction of the com.gocnak.image of equal steps for each
        ...
        do {
            ...
            // Render the incremental scaled com.gocnak.image.
			 ...
			 * incrementalImage to be the new src com.gocnak.image before we iterate around
			 * again to process it down further, we want to flush() the previous
			 * src com.gocnak.image IF (and only IF) it was one of our own temporary
			...
			/*
			 * Now treat our incremental partially scaled com.gocnak.image as the src com.gocnak.image
			 ...
			 * com.gocnak.image with one of our interim BufferedImages so we know when to
			 * explicitly flush the interim "src" on the next cycle through.
			 */
            hasReassignedSrc = true;

            // Track how many times we go through this cycle to scale the com.gocnak.image.
            incrementCount++;
		...
		/*
		 * Once the loop has exited, the src com.gocnak.image argument is now our scaled
		 * result com.gocnak.image that we want to return.
		 */
        return src;
    }
}
